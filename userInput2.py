...
"""
#1
import nltk, tkinter, customtkinter
...
class gui:
    ...
    label=customtkinter.CTkLabel(window,text="Enter your description")
    entry=Entry(window)
    genre_label=customtkinter.CTkLabel(window,text="Enter the genre")
    ...
    def run_comp(): # this gets the input from the entry fields and assigns file paths to global variables for the titles, descriptions and images
       ...
       if genre_input == "Action" or genre_input =="action":
           ...
           vid_path="C:/Users/Ezra/Desktop/SSP/actionVid.txt"
       ...
       if genre_input == "Comedy" or genre_input =="comedy":
           ...
           vid_path="C:/Users/Ezra/Desktop/SSP/comedyVid.txt"
       gui.check=1
       gui.window.destroy()
...
class user_setup(gui):
    ...
    u_button=customtkinter.CTkButton(gui.window,text="click",command= lambda: gui.run_comp())
    u_button.grid(row=2,column=0)
    q_button=customtkinter.CTkButton(gui.window,text="quick",command= lambda: gui.quick_input())
...
class file_setup(gui):
    ...
    opened_video_file=open(vid_path,'r')
